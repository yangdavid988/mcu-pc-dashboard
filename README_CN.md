...
- ✅ **久坐提醒** — PC 超过设定时间未锁屏时，屏幕四边出现呼吸式红色边缘发光，提醒用户起身休息。方案采用 4 条轻量渐变条实现，渲染开销极小。
...
当 PC 在 **Monitor 模式**下持续运行超过设定时间（未锁屏）时，屏幕四边出现**呼吸式红色边缘发光**，以柔和的节奏提醒用户起身休息：

- **呼吸动效** — 24 步 sin² 曲线（200ms/步，4.8s/周期），峰值 OPA 51%，均匀自然、无间断
- **轻量渲染** — 4 条独立渐变条实现（总脏面积 ~61K px），单步渲染 ~30ms，不影响 dashboard 流畅度
- **自动启停** — 基于已有的锁屏/解锁事件（standby 管理器）复位计时；超时进入 Monitor 触发，锁定 PC 后停止
- **跨布局/主题一致** — 布局切换后自动置顶，COBALT / INFERNO / SILICON 三主题均适配
- **时间线** — 进入 Monitor → 计时归零 → 超过阈值 `SEDENTARY_REMINDER_TIMEOUT_MIN` → 在每个 `SEDENTARY_FLASH_CYCLE_SEC` 周期内呼吸 `SEDENTARY_FLASH_ON_SEC` 秒，直至用户锁定 PC
...
| `SEDENTARY_REMINDER_TIMEOUT_MIN` | 45 | 久坐触发阈值（分钟），0=关闭 |
| `SEDENTARY_FLASH_ON_SEC` | 30 | 久坐提醒持续时长（秒） |
| `SEDENTARY_FLASH_CYCLE_SEC` | 60 | 久坐提醒周期（秒） |
